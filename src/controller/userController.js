  ...
  try {
    ...
    if (!isValidPassword(password))
      return res
        ...
        .send({
          status: false,
          message:
            "The password must contain an uppercase, a lowercase, a numeric value, a special character and the limit is from 8 to 15 characters.",
        });
    ...
    if (address) {
      ...
      if (!isValidPincode(address.billing.pincode))
        return res
          .status(400)
          .send({ status: false, message: "Billing pincode is valid" });
    }
...
const loginUser = async function (req, res) {
  try {
    ...
    if (Object.keys(body).length == 0)
      return res
        .status(400)
        .send({ status: false, msg: "Please fill data in body" });
    ...
    if (!isEmail(email))
      return res
        ...
        .send({
          status: false,
          message: "Invalid email, ex.- ( abc123@gmail.com )",
        });
  ...
  try {
    ...
    if (!user) {
      return res
        ...
        .send({
          send: false,
          message: "No profile available with this userId",
        });
    }
    return res
      .status(200)
...
const updateUser = async (req, res) => {
  ...
  if (isValidString(fname)) {
    return res
      .status(400)
      .send({ status: false, message: "fname should be valid" });
  }
  ...
  if (address) {
    try {
      ...
      return res
        .status(400)
        .send({ status: false, message: "please provided valid address" });
    }
    ...
    if (address.shipping) {
      ...
      if (street) {
        if (!isValidate(address.shipping.street))
          return res
            .status(400)
            .send({ status: false, message: "Shipping street is invalid" });
      }
      if (city) {
        if (!isValidate(address.shipping.city))
          return res
            .status(400)
            .send({ status: false, message: "Shipping city is not valid" });
      }
      if (pincode) {
        if (!isValidPincode(address.shipping.pincode))
          return res
    ...
    if (billing) {
      ...
      if (street) {
        if (!isValidate(address.billing.street))
          return res
            .status(400)
            .send({ status: false, message: "Billing street is invalid" });
      }
      if (city) {
  ...
  let update = await userModel.findByIdAndUpdate({ _id: userId }, data, {
    new: true,
  });
